...
func init() {
	...
	replicateCMD.Flags().StringVarP(&groupIDFlag, "group", "g", "", "Consumer Group ID to use for consume")
	replicateCMD.RegisterFlagCompletionFunc("group", groupCompletion)
	replicateCMD.Flags().BoolVarP(&followFlag, "follow", "f", false, "Continue to consume messages until program execution is interrupted/terminated")
	...
	replicateCMD.Flags().Int32SliceVarP(&partitionsFlag, "partitions", "p", []int32{}, "Partitions to consume")
	replicateCMD.Flags().Int32VarP(&messagesCountFlag, "tail", "n", 0, "Print last n messages per partition")
	...
	replicateCMD.Flags().Int32VarP(&partitionFlag, "partition", "p", -1, "Partition to produce to")
}
