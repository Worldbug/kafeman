...
func NewAvroSerializer(
	...
	schemaCache, err := NewSchemaCache(avroSchemaCacheURL)
	if err != nil {
		return nil, err
	}
...
func (as *AvroSerializer) Decode(raw []byte) ([]byte, error) {
	if as.schemaCache != nil {
		return as.schemaCache.DecodeMessage(raw)
	}
...
func (as *AvroSerializer) Encode(input []byte) ([]byte, error) {
	return as.schemaCache.EncodeMessage(as.schemaID, input)
}
